...
def rag_node(state: AgentState) -> dict[str, Any]:
    ...
    harvest_prompt = SystemMessage(
        content=(
            ...
            "La base contient les métadonnées suivantes pour chaque film :\n"
            "titre, synopsis, date de sortie, univers, genres, NOTE, budget (si renseigné),\n"
            "RÉALISATEUR, CASTING (top 5 acteurs), durée, accroche.\n"
            "Elle NE CONTIENT PAS d'informations sur le tournage, la production,\n"
            "la conception, les coulisses ou les anecdotes.\n\n"
            "RÈGLE DE ROUTAGE :\n"
            "-> is_database_sufficient = True  : questions sur titre, date, note, réalisateur,\n"
            "   casting, genres, durée, budget, synopsis, recommandations similaires.\n"
            "-> is_database_sufficient = False : questions sur le tournage, les coulisses,\n"
            "   les anecdotes de production, le making-of, ou tout ce qui n'est pas\n"
            "   une métadonnée de surface."
        )
    )
...
def narration_node(state: AgentState) -> dict[str, Any]:
    ...
    system_prompt = SystemMessage(
        content=(
            ...
            "2. RÉALISATEUR & CASTING : ces informations sont disponibles dans les DONNÉES "
            "LOCALES (colonnes director et cast_top5). Utilise-les directement. Si elles "
            "valent 'non renseigné', déclare les archives muettes — n'invente pas de nom.\n"
            "3. Réponds précisément à ce qui est demandé (date, note, réalisateur, casting, "
            "synopsis, durée, genres, ou infos web disponibles).\n"
            "4. Sois percutant et direct : **1 à 2 paragraphes maximum**.\n"
...
def quality_control_node(state: AgentState) -> dict[str, Any]:
    ...
    audit_prompt = SystemMessage(
        content=(
            ...
            "présents dans les SOURCES ci-dessous. Toute donnée INVENTÉE ou ALTÉRÉE = "
            "verdict 'NON'. Vérifie SPÉCIALEMENT que l'ANNÉE citée est bien celle des "
            "sources : une année absente des sources est une hallucination.\n"
            "2. RÉALISATEUR & CASTING. Ces données sont disponibles dans les SOURCES "
            "LOCALES (champs 'Réalisateur' et 'Casting'). Si le texte cite un réalisateur "
            "ou un acteur, vérifie qu'il correspond aux sources locales OU web. "
            "Un nom inventé absent des deux sources = verdict 'NON'.\n"
